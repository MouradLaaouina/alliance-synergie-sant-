...
import { Menu, X, ChevronRight, Mail, Phone, MapPin, Linkedin, Facebook, Instagram, Youtube, Twitter } from 'lucide-react';
  ...
  return (
    <header className={`fixed w-full z-[1000] transition-all duration-500 ${scrolled || isOpen ? 'bg-white shadow-lg py-3' : 'bg-transparent py-4 md:py-6'}`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
        <Link to="/" className="flex items-center space-x-3 md:space-x-4 group">
          <img
            ...
            alt="A2S - Alliance Synergie Santé, logo officiel"
            width={160}
            height={56}
            decoding="async"
        ...
        {/* Desktop Nav */}
        <nav className="hidden lg:flex items-center space-x-6 xl:space-x-8" aria-label="Navigation principale">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.path}
              to={link.path}
      ...
      <div
        ...
        aria-modal="true"
        aria-hidden={!isOpen}
        className={`lg:hidden fixed inset-0 bg-white z-[99] transition-transform duration-500 ease-in-out transform ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
      >
  ...
  return (
    <BrowserRouter>
      ...
      <a
        ...
        className="sr-only focus:not-sr-only focus:absolute focus:left-4 focus:top-4 focus:z-[1200] focus:bg-white focus:text-slate-900 focus:px-4 focus:py-2 focus:rounded-lg focus:shadow-lg"
      >
